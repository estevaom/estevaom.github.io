...
(function () {
  ...
  function initStarryNightTheme() {
    ...
    createBrushStrokes(
      starryNightConfig.brushStrokes ? starryNightConfig.brushStrokes.count : 0,
    );
  ...
  function reduceEffectsForMobile() {
    ...
    starryNightConfig.stars.count = Math.floor(
      starryNightConfig.stars.count * 0.6,
    );
    starryNightConfig.swirls.count = Math.floor(
      starryNightConfig.swirls.count * 0.5,
    );
    starryNightConfig.shootingStars.count = Math.floor(
      starryNightConfig.shootingStars.count * 0.5,
    );
    starryNightConfig.fireflies.count = Math.floor(
      starryNightConfig.fireflies.count * 0.5,
    );
    starryNightConfig.brushStrokes.count = Math.floor(
      starryNightConfig.brushStrokes.count * 0.5,
    );
  }
  ...
  function createShootingStarKeyframes() {
    ...
    for (var fadePoint = 75; fadePoint <= 90; fadePoint += 5) {
      if (
        starryNightConfig.shootingStars.direction === "left-to-right" ||
      ...
      if (
        starryNightConfig.shootingStars.direction === "right-to-left" ||
        starryNightConfig.shootingStars.direction === "random" ||
        starryNightConfig.shootingStars.direction === "multi-position"
      ) {
  ...
  function createStars(count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      var size = randomBetween(
        starryNightConfig.stars.minSize,
        starryNightConfig.stars.maxSize,
      );
  ...
  function createSwirls(count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      var size = randomBetween(
        starryNightConfig.swirls.minSize,
        starryNightConfig.swirls.maxSize,
      );
  ...
  function createShootingStars(count) {
    ...
    for (var i = 0; i < starsToCreate; i++) {
      ...
      var duration = randomBetween(
        starryNightConfig.shootingStars.minDuration,
        starryNightConfig.shootingStars.maxDuration,
      );
      ...
      var thickness = Math.round(
        starryNightConfig.shootingStars.thickness * thicknessVariation,
      );
      var length = Math.round(
        starryNightConfig.shootingStars.length *
          (thicknessVariation * 0.8 + 0.2),
      ); // Size affects length but less dramatically
      ...
      if (direction === "left-to-right") {
        ...
        x = randomBetween(
          0,
          window.innerWidth *
            (starryNightConfig.shootingStars.startSidePosition / 100),
        );
        y = randomBetween(
          window.innerHeight *
            (starryNightConfig.shootingStars.startAreaTop / 100),
          window.innerHeight *
            (starryNightConfig.shootingStars.startAreaBottom / 100),
        );
        ...
        x = randomBetween(
          window.innerWidth *
            (1 - starryNightConfig.shootingStars.startSidePosition / 100),
          window.innerWidth,
        );
        y = randomBetween(
          window.innerHeight *
  ...
  function createFireflies(count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      var size = randomBetween(
        starryNightConfig.fireflies.minSize,
        starryNightConfig.fireflies.maxSize,
      );
      var x = randomBetween(10, window.innerWidth - 10);
  ...
  function createBrushStrokes(count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      var width = randomBetween(
        starryNightConfig.brushStrokes.minWidth,
        starryNightConfig.brushStrokes.maxWidth,
      );
  ...
  function createEnergyWave(starElement) {
    ...
    var size = randomBetween(
      starryNightConfig.energyWaves.minSize,
      starryNightConfig.energyWaves.maxSize,
    );
  ...
  function scheduleRandomEffects() {
    ...
    var shootingStarInterval = setInterval(function () {
      if (
        activeShootingStars < 10 &&
        Math.random() > 1 - starryNightConfig.shootingStars.chance
      ) {
  ...
  function createMoon() {
    ...
    moon.style.background =
      "radial-gradient(circle at 30% 30%, #fffce3, #ffed91 40%, #ffd25b 70%)";
  ...
  function createHugeStars(count) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      var size = randomBetween(
        starryNightConfig.hugeStars.minSize,
        starryNightConfig.hugeStars.maxSize,
      );
      ...
      var movementDuration = randomBetween(
        starryNightConfig.hugeStars.minSpeed,
        starryNightConfig.hugeStars.maxSpeed,
      );
      ...
      star.style.background =
        "radial-gradient(circle at 30% 30%, #ffffff, #fffce3 30%, #ffd25b 70%)";
      star.style.boxShadow = "0 0 35px 10px rgba(255, 255, 200, 0.7)";
      star.style.animation =
        "huge-star-pulse 5s ease-in-out infinite, huge-star-glow 7s ease-in-out infinite";
      ...
      glow.style.background =
        "radial-gradient(circle at center, rgba(255, 250, 150, 0.4) 0%, rgba(255, 240, 110, 0.2) 40%, rgba(255, 230, 100, 0) 70%)";
      glow.style.zIndex = "-1";
      ...
      for (var r = 0; r < ringCount; r++) {
        ...
        ring.style.border =
          "1px solid rgba(255, 250, 150, " + (0.3 - r * 0.07) + ")";
      ...
      for (var j = 0; j < rayCount; j++) {
        ...
        ray.style.background =
          "linear-gradient(90deg, rgba(255, 250, 150, 0.5) 0%, rgba(255, 230, 100, 0.1) 60%, rgba(255, 220, 80, 0) 100%)";
